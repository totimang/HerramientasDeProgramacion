...
struct automovil
{
	...
	char tipo[20];
	int modelo;
};//no le asigno un nombre

struct coche
{
	struct automovil info_coche; //aqui le asigno un nombre a la estructura automovil
	int cant_ruedas;
	char usado[3];
	int km;
}autos[t];
...
int main()
{
	...
	for(int i=0; i<t; i++)
	{
		...
		mod:
			...
			cin>>a;
			if (a==1) strcpy(autos[i].info_coche.tipo,"SEDAN");
			else if (a==2) strcpy(autos[i].info_coche.tipo,"COUPE");
			else if (a==3) strcpy(autos[i].info_coche.tipo,"CABRIOLET");
			else if (a==4) strcpy(autos[i].info_coche.tipo,"RURAL");
			else
			{
				printf("%d NO es una opcion valida presione 1 a 4: \n\n");
				goto mod;
			}
			...
			while (res!= "si" && res!= "no")
			{
				cout<<"RESPUESTA INCONRRECTA INGRESE 'si' O 'no' : "<<endl;
				cin.ignore();
				cin>>res;
			}
